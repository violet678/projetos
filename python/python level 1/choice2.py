...
def atacar_inimigo_escolha():
    escolha_ataque = input('como voce quer atacar seu inimigo? combate ou magia').upper
...
#primeiro turno de ataque ⚔️

escolha_ataque = input("Atacar com (1) Combate ou (2) Magia? ").strip().lower()

if escolha_ataque == '1' or escolha_ataque == 'combate':
    atacar_inimigo('zumbi')
elif escolha_ataque == '2' or escolha_ataque == 'magia':
    atacar_magia('zumbi')
else:
    print("Opção inválida! Atacando com combate padrão")
    atacar_inimigo('zumbi')
if ataque == 2:  # Esqueleto
    while inimigos['esqueleto']['vida'] > 0 and vida > 0:
        atacar_inimigo('esqueleto')
        if inimigos['esqueleto']['vida'] <= 0:
            print("Você derrotou o esqueleto!")
            aumento_de_xp(3)
            inimigo_derrotado = 'esqueleto'  # ✅ Define o inimigo derrotado
            break
        # Inimigo contra-ataca aleatoriamente
        inimigo_ataca(random.choice(['zumbi', 'slime']))
        
elif ataque == 3:  # Slime
    while inimigos['slime']['vida'] > 0 and vida > 0:
        atacar_inimigo('slime')
        if inimigos['slime']['vida'] <= 0:
            print("Você derrotou o slime!")
            aumento_de_xp(3)
            inimigo_derrotado = 'slime'  # ✅ Define o inimigo derrotado
            break
        # Inimigo contra-ataca aleatoriamente
        inimigo_ataca(random.choice(['zumbi', 'esqueleto']))
    

#segundo turno de ataque ⚔️

#se tiver derrotado o zumbi 🧟
 #Segundo turno (Esqueletos/Slimes)
if inimigo_derrotado == 'zumbis':
...
#se tiver derrotado o esqueleto 💀
if inimigo_derrotado == 'esqueleto':
    print("\nRestam zumbis e slimes. Qual você quer atacar?")
    print("1 - Zumbis | 2 - Slimes")
    ataque2 = int(input("Escolha: "))

    if ataque2 == 1:  # Zumbi
        while inimigos['zumbi']['vida'] > 0 and vida > 0:
            atacar_inimigo('zumbi')
            if inimigos['zumbi']['vida'] <= 0:
...
if compra == 'flecha':
    ...
    print('voce não tem dinheiro para comprar a adaga agora')
